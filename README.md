...
| AWS_ACCOUNT_ID | String | "000000000000" | The default account streams are namespaced under. A single instance can serve multiple accounts: a request selects its account by signing with the 12-digit account id as its AWS access key id (parsed from the `Authorization` header or `X-Amz-Credential` query param). Any other access key id falls back to this default, so existing single-account usage is unchanged. |
