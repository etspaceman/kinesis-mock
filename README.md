...
| AKID_TO_ACCOUNT_ID | String | | A comma-delimited string of `AKID:accountId` pairs that map AWS Access Key IDs to account IDs for multi-account namespacing. The AKID is parsed from the request's `Authorization` header (or `X-Amz-Credential` query param). AKIDs not present in this map fall back to `AWS_ACCOUNT_ID`. For example: "AKIA1:111111111111,AKIA2:222222222222" |
