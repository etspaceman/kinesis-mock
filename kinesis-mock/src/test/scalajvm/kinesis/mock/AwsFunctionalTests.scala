...
import munit.{CatsEffectFunFixtures, CatsEffectSuite, Tag as MUnitTag}
...
trait AwsFunctionalTests extends CatsEffectSuite with CatsEffectFunFixtures:
  ...
  protected val initializedStreams: Vector[(String, Int)] = Vector(
    "stream1" -> 3,
    "stream2" -> 2,
    "stream3" -> 1,
    "stream4" -> 2,
        ...
        Gen
          .oneOf(
            AwsRegion.values
              ...
              .filter(x => Try(Region.of(x.entryName)).isSuccess)
          )
          .one
      )
    )
