...
class PutResourcePolicyTests
  ...
  test("It should store a policy for a stream ARN")(PropF.forAllF {
    (streamArn: StreamArn) =>
      ...
      for
        ...
        req = PutResourcePolicyRequest(streamArn.streamArn, policy)
  ...
  test("It should store a policy for a consumer ARN")(PropF.forAllF {
    (streamArn: StreamArn, consumerName: ConsumerName) =>
      ...
      for
        ...
        streamsRef <- Ref.of[IO, Streams](streams)
        req = PutResourcePolicyRequest(consumer.consumerArn.consumerArn, policy)
        res <- req.putResourcePolicy(streamsRef)
        s <- streamsRef.get
      yield assert(
        res.isRight && s.resourcePolicies
  ...
  test("It should reject a malformed ARN with InvalidArgumentException"):
    val policy = "{}"
    for
      streamsRef <- Ref.of[IO, Streams](Streams.empty)
      req = PutResourcePolicyRequest("not-an-arn", policy)
      res <- req.putResourcePolicy(streamsRef)
    yield assert(
